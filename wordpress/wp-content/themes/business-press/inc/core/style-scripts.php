...
if( !function_exists( 'business_press_scripts' ) )
{
	function business_press_scripts()
	{
		...
		*/
		...
		// Load side bar menu js depends on jquery and if enabled by customizer
		if( get_theme_mod( 'sb_menu_onoff', '1' ) == 1 )
		{
			wp_enqueue_script( 'business-press-sidebarmenu', get_template_directory_uri() . '/js/sidebarmenu.js', array( 'jquery' ), $business_press_version, true );
		}

		// CSP Search js depends on jquery and if enabled by customizer
		if( get_theme_mod( 'top_bar_seach_icon', '1' ) == 1 && get_theme_mod( 'business_press_display_top_bar', '1' ) == 1 )
		{
			wp_enqueue_script( 'business-press-csp-search', get_template_directory_uri() . '/js/scpsearch.js', array( 'jquery' ), $business_press_version, true );
		}
